...
* **Testing & Coverage:** `pytest`, `pytest-cov`, FastAPI `TestClient`, `unittest.mock`
...
* **[Day 10] Containerization & Multi-Service Architecture:** Orchestrating PostgreSQL/pgvector, FastAPI, and Streamlit via Docker Compose with environment-agnostic configuration (`os.getenv`).
* **[Day 11] Production-Grade Automated Testing (`pytest`):** Implementing robust unit and endpoint test suites using FastAPI's `TestClient`, mocking external ML models, databases, and LLMs for fast, reliable verification.

## 📂 3. Project Structure

* `docker-compose.yml` — Containerized multi-service PostgreSQL, FastAPI, and Streamlit stack
...
* `test_app.py` — Day 11: Automated `pytest` suite with database and model mocking
...
```bash
# 1. Start PostgreSQL vector database
